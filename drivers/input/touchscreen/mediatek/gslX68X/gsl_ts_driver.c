...
static void gsl_sw_init(struct i2c_client *client)
{
	...
	gsl_io_control(client);
	//gsl_load_fw(client,gsl_cfg_table[gsl_cfg_index].fw,gsl_cfg_table[gsl_cfg_index].fw_size); //Disable load Firmware to controller
	gsl_io_control(client);
...
identify_tp_repeat:
	...
	//gsl_load_fw(ts->client,GSL_IDT_FW,num); disable load firmware to controller
